# 👋 Hi, I'm Tanzeel Yousef  

🚀 **Available for Hire**

I am a **Full Stack Developer & WordPress Specialist** who architects complete web solutions — from beautiful frontend interfaces in **React.js** to powerful backend systems using **Node.js & MySQL**, along with robust custom **WordPress** platforms.

I build fast, scalable, SEO-optimized, and business-focused websites & web applications.

---

## 💼 My Journey

My journey started with **WordPress Development**.  
I successfully delivered **50+ projects**, mastering:

- Custom WordPress Development  
- WooCommerce Stores  
- API Integrations  
- Performance Optimization  
- Security & SEO  

But I didn’t stop there.

I transitioned into **Full Stack Development**, where I now build:

- ⚛ Modern React.js Frontends  
- 🔗 RESTful APIs using Node.js & Express  
- 🗄 MySQL Database Architectures  
- 🧠 Custom Booking Systems & Directories  
- 🔍 Multi-field Advanced Search Systems  

Now, I create complete custom web applications from scratch — frontend to backend.

---

# 📊 Professional Stats

- ✅ 50+ Projects Delivered  
- 💼 3+ Years Experience  
- 🤝 20+ Happy Clients  
- 🚀 30+ Seamless Website Migrations  

---

# 🛠 My Services

### 🔹 WordPress Development
- Custom Themes & Functionality  
- WooCommerce Setup  
- Elementor & Elementor Pro Expert  
- Malware Removal & Security  
- Speed Optimization  

### 🔹 React & Frontend Development
- Single Page Applications (SPA)  
- Responsive UI Development  
- Pixel-Perfect Figma to Code  
- Performance Optimization  

### 🔹 Backend Development
- Node.js & Express  
- REST API Development  
- MySQL Database Design  
- Authentication Systems  

### 🔹 E-Commerce Solutions
- WooCommerce Stores  
- Payment Gateway Integration (Stripe, PayPal)  
- Inventory & Order Automation  
- Membership Systems  

### 🔹 SEO & Optimization
- On-Page SEO  
- Technical SEO  
- Schema Markup  
- Google Analytics & Search Console Setup  

---

# ⚙ Tech Stack

### 💻 Frontend
- HTML5  
- CSS3  
- JavaScript (ES6+)  
- React.js  
- Bootstrap  

### 🖥 Backend
- Node.js  
- Express.js  
- MySQL  
- REST APIs  

### 🌐 CMS & Platforms
- WordPress  
- WooCommerce  
- Elementor / Elementor Pro  

### 🛠 Tools
- Git & GitHub  
- VS Code  
- Vercel / Netlify  
- NPM  

---

# 🏢 Professional Experience

## WordPress Developer & Full Stack Developer  
**TECH BITS IT**  
📍 Dera Ismail Khan, Pakistan  
📅 April 2021 – Present  

### Key Responsibilities:
- Delivered 50+ WordPress projects  
- Increased website speed by up to 60%  
- Integrated Stripe & PayPal payment systems  
- Built vehicle registration & ULEZ detection systems  
- Created advanced booking & membership platforms  
- Developed full stack React + Node.js applications  

---

# 🎓 Education

**Bachelor of Science in Computer Science**  
Hayat Medical College  
2025 (In Progress)

---

# 🚀 Selected Project Categories

- ⚛ React & Full Stack Applications  
- 🏢 Law & Business Directories  
- 🚗 Automotive Platforms  
- 🛍 E-Commerce Stores  
- 🧠 Custom WordPress Solutions  
- 📈 SEO-Focused Business Websites  

---

# 🌐 Portfolio

🔗 https://mtwebexperts.site  

---

# 🔗 Professional Profiles

[![Website](https://img.shields.io/badge/Website-MT%20Web%20Experts-black?style=for-the-badge&logo=google-chrome)](tanzeel-yousef.netlify.app)  
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Tanzeel%20Yousef-blue?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/muhammad-tanzeel-ur-rehman-04b898355/)  
[![Fiverr](https://img.shields.io/badge/Fiverr-Profile-success?style=for-the-badge&logo=fiverr)](https://www.fiverr.com/tanzeelyousef96?public_mode=true)  
[![Upwork](https://img.shields.io/badge/Upwork-Profile-success?style=for-the-badge&logo=upwork)](https://www.upwork.com/freelancers/~019473e56b1a594fa8?mp_source=share)  

---

# 📬 Contact

📧 Email: mtwebexperts.site@gmail.com  
📱 WhatsApp: 0318-9046142  
📍 Dera Ismail Khan, Pakistan  

---

## 💬 Quote I Believe In

> “First, solve the problem. Then, write the code.”  
> — John Johnson  

---
